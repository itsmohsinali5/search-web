...
export const Home = () => {
  ...
  useEffect(() => {
    ...
    axios.get('http://api.travelpayouts.com/data/en/airports.json')
    .then((response) => {
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={{ width: "90%" }}>
        ...
        <View style={{ zIndex: open || openDes ? 0 : 1, marginTop: 10 }}>
          <Text style={{...styles.title, color: Color.blackColor, fontSize: 16}}>Select Passengers</Text>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
            <Text style={styles.text}>adults</Text>
            <View style={{ alignItems: 'center', flexDirection: 'row' }}>
                <Text style={{...styles.text, marginRight: 10}}>{pessanger.adults ? pessanger.adults : 0}</Text>
                    <TouchableOpacity style={{ marginRight: 5 }} onPress={() => {
                        if (pessanger.adults > 0) {
                            setPessanger(prevState => ({ ...prevState, adults: prevState.adults - 1 }));
                        }
                    }}>
                        <AntDesign name="minuscircleo" size={22} color={Color.redColor} />
                    </TouchableOpacity>
                <TouchableOpacity onPress={() => setPessanger(prevState => ({ ...prevState, adults: prevState.adults + 1 }))}>
                    <Ionicons name="add-circle-outline" size={26} color={Color.blueColor} />
                </TouchableOpacity>
            </View>
          </View>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <View style={{ alignItems: 'center', flexDirection: 'row' }}>
                <Text style={{...styles.text, marginRight: 10}}>{pessanger.children ? pessanger.children : 0}</Text>
                <TouchableOpacity style={{ marginRight: 5 }} onPress={() => {
                        if (pessanger.children > 0) {
                            setPessanger(prevState => ({ ...prevState, children: prevState.children - 1 }));
                        }
                    }}>
                        <AntDesign name="minuscircleo" size={22} color={Color.redColor} />
                </TouchableOpacity>
                <TouchableOpacity onPress={() => setPessanger(prevState => ({ ...prevState, children: prevState.children + 1 }))}>
                    <Ionicons name="add-circle-outline" size={26} color={Color.blueColor} />
                </TouchableOpacity>
            </View>
          </View>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <View style={{ alignItems: 'center', flexDirection: 'row' }}>
                <Text style={{...styles.text, marginRight: 10}}>{pessanger.infants ? pessanger.infants : 0}</Text>
                <TouchableOpacity style={{ marginRight: 5 }} onPress={() => {
                        if (pessanger.infants > 0) {
                            setPessanger(prevState => ({ ...prevState, infants: prevState.infants - 1 }));
                        }
                    }}>
                        <AntDesign name="minuscircleo" size={22} color={Color.redColor} />
                    </TouchableOpacity>
                <TouchableOpacity onPress={() => setPessanger(prevState => ({ ...prevState, infants: prevState.infants + 1 }))}>
                    <Ionicons name="add-circle-outline" size={26} color={Color.blueColor} />
                </TouchableOpacity>
            </View>
          </View>
